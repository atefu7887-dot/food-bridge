const express = require('express');
const router = express.Router();
const Donation = require('../models/Donation');
const User = require('../models/User');

// ==========================================
// 1. مسارات المتبرع (Donor)
// ==========================================

// ➕ إضافة تبرع جديد
router.post('/add', async (req, res) => {
    try {
        const { donorId, itemDetails, quantity, location, expiryDate, receiverId } = req.body;

        const donor = await User.findById(donorId);
        if (!donor || donor.role !== 'Donor') {
            return res.status(400).json({ success: false, message: 'Invalid donor or insufficient permissions' });
        }

        const newDonation = new Donation({
            donor: donorId,
            receiver: receiverId || null,
            itemDetails,
            quantity,
            location,
            expiryDate
        });

        await newDonation.save();
        res.status(201).json({ success: true, message: 'Donation added successfully', donation: newDonation });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 👤 عرض تاريخ تبرعات المتبرع نفسه
router.get('/donor-donations/:donorId', async (req, res) => {
    try {
        const donations = await Donation.find({ donor: req.params.donorId })
            .populate('receiver', 'username email')
            .populate('driver', 'username phone')
            .sort({ createdAt: -1 });
        res.status(200).json({ success: true, donations });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// ==========================================
// 2. مسارات الجمعية (Receiver / NGO)
// ==========================================

// 🔍 عرض التبرعات المتاحة في السوق (التي ليس لها جمعية بعد)
router.get('/available-for-ngo', async (req, res) => {
    try {
        const donations = await Donation.find({ status: 'Pending', receiver: null })
            .populate('donor', 'username phone location');
        res.status(200).json({ success: true, donations });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 📥 حجز التبرع بواسطة الجمعية
router.patch('/:id/request-donation', async (req, res) => {
    try {
        const { receiverId } = req.body;
        const donation = await Donation.findById(req.params.id);

        if (!donation || donation.receiver !== null) {
            return res.status(400).json({ success: false, message: 'Donation unavailable' });
        }

        donation.receiver = receiverId;
        await donation.save();
        res.status(200).json({ success: true, message: 'Donation reserved by NGO', donation });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 🏠 عرض التبرعات التي حجزتها الجمعية بالفعل
router.get('/receiver-donations/:receiverId', async (req, res) => {
    try {
        const donations = await Donation.find({ receiver: req.params.receiverId })
            .populate('donor', 'username phone')
            .populate('driver', 'username phone');
        res.status(200).json({ success: true, donations });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 🚚 جلب قائمة السائقين المتاحين للجمعية لتختار منهم
router.get('/available-drivers', async (req, res) => {
    try {
        const drivers = await User.find({ role: 'Driver' }).select('username phone email');
        res.status(200).json({ success: true, drivers });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 🎯 تعيين سائق محدد لطلب معين (بواسطة الجمعية)
router.patch('/:id/assign-driver', async (req, res) => {
    try {
        const { driverId, receiverId } = req.body;
        const donation = await Donation.findById(req.params.id);

        if (!donation) return res.status(404).json({ success: false, message: 'Not found' });
        
        // التأكد أن هذه الجمعية هي صاحبة الحق في تعيين السائق لهذا الطلب
        if (donation.receiver.toString() !== receiverId) {
            return res.status(403).json({ success: false, message: 'Unauthorized' });
        }

        donation.driver = driverId;
        donation.status = 'Assigned'; // تحويل الحالة فور اختيار السائق
        await donation.save();

        res.status(200).json({ success: true, message: 'Driver assigned successfully', donation });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// ==========================================
// 3. مسارات السائق (Driver)
// ==========================================

// 📋 عرض الطلبات الموجهة للسائقين (Pending ولها جمعية) أو الموجهة لسائق بعينه
router.get('/available-requests', async (req, res) => {
    try {
        const donations = await Donation.find({ 
            status: 'Pending', 
            receiver: { $ne: null } 
        })
        .populate('donor', 'username phone')
        .populate('receiver', 'username address');
        res.status(200).json({ success: true, donations });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 🤝 قبول الطلب العام (إذا لم يكن معيناً لسائق محدد)
router.put('/:id/accept', async (req, res) => {
    try {
        const { driverId } = req.body;
        const donation = await Donation.findById(req.params.id);

        if (donation.status !== 'Pending') {
            return res.status(400).json({ success: false, message: 'Already assigned' });
        }

        donation.driver = driverId;
        donation.status = 'Assigned';
        await donation.save();

        res.status(200).json({ success: true, donation });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

// 🔄 تحديث الحالة (تم الاستلام أو تم التوصيل)
router.patch('/:id/update-status', async (req, res) => {
    try {
        const { status, driverId } = req.body;
        const donation = await Donation.findOne({ _id: req.params.id, driver: driverId });

        if (!donation) return res.status(404).json({ success: false, message: 'Access denied' });

        donation.status = status;
        await donation.save();

        res.status(200).json({ success: true, message: `Status: ${status}`, donation });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});

module.exports = router;